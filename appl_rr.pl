...
#my $num_replicate = $num_cores - (keys %_cores);
...
while(my $reception = $fork_class->{'ownedReception'}[++$receptionI]) {
	...
	if($args ne '') { $args .= ', '; $transition_args .= ', '; }
	$args .= $reception->{'ownedParameter'}[$i]->{'name'};
	...
	$body .= 'if(self->' . $reception->{'name'} . '_roundrobin == ' . $i . ') {' . "\n";
	$body .= "\t" . $interface_for_fork->{'name'} . '__' . $reception->{'name'} . '(self->out_' . $i . ', ' . $args . ');' . "\n";
	$body .= '}' . "\n";
    }
    $body .= 'if(++(self->' . $reception->{'name'} . '_roundrobin) >= ' . $num_replicate . ') {' . "\n";
    $body .= "\t" . 'self->' . $reception->{'name'} . '_roundrobin = 0;' . "\n";
    $body .= '}' . "\n";
...
while(my $operation = $interface_for_join->{'ownedOperation'}[++$operationI]) {
	...
	if($args ne '') { $args .= ', '; }
	$args .= $operation->{'ownedParameter'}[$i]->{'name'};
    }

    my $body = $interface_for_join->{'name'} . '__' . $operation->{'name'} . '(self->out, ' . $args . ');' . "\n";
